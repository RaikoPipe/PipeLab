...
def render_corner(pos:vpy.vector, scene: vpy.canvas, course:tuple, opacity:float, corner_data:odc.pipe_data = pipe_corner) -> list:
    # prototype corner
    ...
    v_pipe = vpy.cylinder(scene=scene, size=size, color=corner_data.color, axis=vpy.vector(0, 1, 0), pos= pos, opacity=opacity)
...
# todo: handle corners
def render_pipe(scene : vpy.canvas, pos: vpy.vector, direction: tuple[int,int], opacity:float, pipe_data: odc.pipe_data = default_pipes[1], part_id: Optional[int] = None,
                )\
        -> vpy.cylinder:
