...
def send_new_pick_event(part_id, process_planner:ProcessPlanner, tree, part_stock_tree, style_grid, previous_style_grids):
...
class function_test_app:
    def __init__(self, state_grid: np.ndarray, path_problem: PathProblem, initial_state: Optional[State]):
        ...
        style.configure(fit_caution_style, background="cyan", foreground="yellow", width=7, height=7, font=('Helvetica', 8))
        style.configure(att_caution_style, background="magenta", foreground="yellow", width=7, height=7,
                        font=('Helvetica', 8))
        style.configure(pipe_caution_style, background="blue", foreground="yellow", width=7, height=7,
                        font=('Helvetica', 8))
        style.configure(fit_warn_style, background="cyan", foreground="red", width=7, height=7, font=('Helvetica', 8))
        style.configure(att_warn_style, background="magenta", foreground="red", width=7, height=7,
                        font=('Helvetica', 8))
        style.configure(pipe_warn_style, background="blue", foreground="red", width=7, height=7,
                        font=('Helvetica', 8))
        ...
        solution_button_grid,_,_ = get_button_grid(state_grid=state_grid, button_grid_frame=solution_button_grid_frame,
        ...
        construction_button_grid, style_grid, previous_style_grids = get_button_grid(state_grid=self.process_planner.latest_state.state_grid, start=start,
        ...
        return_to_previous_state = ttk.Button(tool_frame, text="Undo action",
                                      command=lambda : undo_action(process_planner=self.process_planner, part_stock_tree=part_stock_tree, button_grid=construction_button_grid,style_grid=style_grid,
