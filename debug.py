...
transition_points_set = {
    (-2, 16)}  #: Set containing transition points that show where a transition from one wall to another occurs.

start_pos = (0, 1)  #: Start position of the search.
goal_pos = (0, 24)  #: Goal position that needs to be reached

start_directions = {(0, 1), (1, 0), (-1, 0), (
0, -1)}  #: Set containing directions that restrict in which direction the search algorithm can start the search.
goal_directions = {(0, 1), (1, 0), (-1, 0), (
0, -1)}  #: Set containing directions that restrict in which direction the search algorithm can append to the goal.
...
state_grid = grid_functions.get_empty_stategrid(x,
                                                y)  #: A grid array where each position in the grid points to an integer value representing a state.

algorithm = "mcsa*"  #: Defines how scores are calculated and therefore how nodes are evaluated in the search.
weights = Weights(path_length=1, cost=1,
                  distance_to_obstacles=0)  #: Weights used if search algorithm is a multi-criteria search algorithm (mca*, mcsa*)
