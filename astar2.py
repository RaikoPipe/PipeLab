...
def partHeuristic(length, current, currentNeighbor, goal):
    cost = priceList[length-1]

    distance = length * cost

    neighToGoalDistance = np.abs(goal[0] - currentNeighbor[0]) + np.abs(
        goal[1] - currentNeighbor[1])  # manhattan distance from neigh to goal
...
def astar(array, start, goal, shiftpos, startAxis, goalAxis, testingPath,testedPath, heuristicType, weight,  yDots, pipeTypeDict, unlimited_parts):
    ...
    while oheap:
        ...
        for i, j in nextNeighbors:
            ...
            tentative_g_score = gscore[current] + heuristic(current, current_neighbor)

            if heuristicType == "price-based":
                tentative_g_score = gscore[current] + partHeuristic(nextNeighbors.get((i, j)), current,
                                                                     current_neighbor, goal)
            else:
                tentative_g_score = gscore[current] + modified_heuristic(current, current_neighbor,
                                                                          current_neighbor, heuristicType, weight,
                                                                          goal)
            ...
            if tentative_g_score < gscore.get(neighbor, 0) or neighbor not in [i[1] for i in oheap]:
                ...
                fscore[neighbor] = tentative_g_score + modified_heuristic(current_neighbor, goal, current, heuristicType, weight, goal)
